...
// Hämtar nivå selector-elementet
...
function draw() {
  ...
  // ritar ut blocken
  canvas.fillStyle = "rgb(152, 116, 62)";
  for (let i = 0; i < blocks.length; i++) {
    canvas.fillRect(blocks[i].x,blocks[i].y, cellSize, cellSize);
  }
...
function generateObjects(array, amount) {
  while (array.length < amount) {
    // slumpar fram ett x- och y-värde
    const object = {
      x: Math.floor(Math.random() * gridWidth) * cellSize,
      y: Math.floor(Math.random() * gridHeight) * cellSize,
    }

    if (selectedLevel == 3 && array == food) {
      object.timer = 20;
    }
...
function resetPositions() {
  ...
  blocks.splice(0);
  if (selectedLevel == 2) {
    generateObjects(blocks, 10);
  } 

  // Tar bort all mat
  food.splice(0);
  
  // Om nivån är 3 skapas ny mat med en timer, annars skapas den utan det
  if (selectedLevel == 3) {
    generateObjects(food, foodAmount);
  } else {
    generateObjects(food, foodAmount);
  }
}
